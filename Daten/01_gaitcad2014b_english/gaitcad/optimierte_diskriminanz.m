  function [phi_dis,d]=optimierte_diskriminanz(phi_dis,d_org,code,einstellungen,uihd)
% function [phi_dis,d]=optimierte_diskriminanz(phi_dis,d_org,code,einstellungen,uihd)
%
% einstellungen=[] bedeutet Ruf durch KAFKA...
%
% The function optimierte_diskriminanz is part of the MATLAB toolbox Gait-CAD. 
...
if nargin<4 %KAFKA-Ruf
   ...
   global Globaluihd
   tmpstring=get(Globaluihd(11,85),'string');
   verfahren=tmpstring(get(Globaluihd(11,85),'value'),:);       
   
else %Vorbereitung für Davedesign
   % Sollen durch Drehungen der Transmatrix neue Startwerte berechnet werden?
   konvergenz_mit_drehung = (get(uihd(45),'value')==1);								
...
if ~exist('phi_dis', 'var')
   ...
   if konvergenz_mit_drehung
      while konvergenz_mit_drehung
         ...
         phi_dis=optimiere(phi_dis,change_param, code, d_org, Schrittweite, verfahren, 1, Qend);					
