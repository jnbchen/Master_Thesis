...
if nargin < 3, myerror('Not enough arguments!'); end
...
if ~isreal(X)
  myerror('Input has an imaginary part.');
end
...
switch lower(s_verbose)
 ...
 case 'off'
  b_verbose = 0;
 otherwise
  myerror(sprintf('Illegal value [ %s ] for parameter: ''verbose''\n', s_verbose));
end
...
switch lower(approach)
 ...
 case 'defl'
  approachMode = 2;
 otherwise
  myerror(sprintf('Illegal value [ %s ] for parameter: ''approach''\n', approach));
end
...
if vectorSize < numOfIC
  myerror('Must have numOfIC <= Dimension!');
end
...
switch lower(g)
 ...
 case 'skew'
  gOrig = 40;
 otherwise
  myerror(sprintf('Illegal value [ %s ] for parameter: ''g''\n', g));
end
if sampleSize ~= 1
  gOrig = gOrig + 2;
end
...
switch lower(finetune)
 ...
 case 'off'
  ...
  finetuningEnabled = 0;
 otherwise
  myerror(sprintf('Illegal value [ %s ] for parameter: ''finetune''\n', ...
        finetune));
end
...
switch lower(stabilization)
 ...
 otherwise
  myerror(sprintf('Illegal value [ %s ] for parameter: ''stabilization''\n', ...
        stabilization)); 
end
...
switch lower(initState)
 ...
 otherwise
  myerror(sprintf('Illegal value [ %s ] for parameter: ''initState''\n', initState));
end
...
switch lower(displayMode)
 ...
 otherwise
  myerror(sprintf('Illegal value [ %s ] for parameter: ''displayMode''\n', displayMode));
end
...
if approachMode == 1,
  ...
  for round = 1:maxNumIterations + 1,
    ...
    switch usedNlinearity
     ...
     otherwise
      myerror('Code for desired nonlinearity not found!');
    end
  end
