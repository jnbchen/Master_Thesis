...
switch lower(s_interactive)
 ...
 case 'gui'
  b_interactive = 2;
 otherwise
  myerror(sprintf('Illegal value [ %s ] for parameter: ''interactive''\n', ...
        s_interactive));
end
...
if ~(b_interactive)
  if lastEig < 1 | lastEig > oldDimension
    myerror(sprintf('Illegal value [ %d ] for parameter: ''lastEig''\n', lastEig));
  end
  if firstEig < 1 | firstEig > lastEig
    myerror(sprintf('Illegal value [ %d ] for parameter: ''firstEig''\n', firstEig));
  end
end
