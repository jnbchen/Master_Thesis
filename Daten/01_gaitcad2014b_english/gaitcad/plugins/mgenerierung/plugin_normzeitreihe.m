  function [datenOut, ret, info] = plugin_normzeitreihe(paras, datenIn)
% function [datenOut, ret, info] = plugin_normzeitreihe(paras, datenIn)
%
% Berechne die Beschleunigungszeitreihen
% Plugin-Fkt für Gait-CAD.
%
% The function plugin_normzeitreihe is part of the MATLAB toolbox Gait-CAD. 
...
anz_zr = 1;
info = struct('beschreibung', 'Norm time series', 'bezeichner', 'NDTS', 'anz_zr', anz_zr, 'anz_em', 0, 'laenge_zr', paras.par.laenge_zeitreihe, 'typ', 'TS');
info.einzug_OK = 0; 
info.richtung_entfernen = 0;
info.anz_benoetigt_zr = 1;

info.explanation      = 'computes the deviation from a time series to a norm time series as a new time series.';
info.explanation_long = '(Eq. (3.9) in \cite{Loose04})';
