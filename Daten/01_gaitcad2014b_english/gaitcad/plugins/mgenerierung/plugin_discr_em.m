  function [datenOut, ret, info] = plugin_discr_em(paras, datenIn)
% function [datenOut, ret, info] = plugin_discr_em(paras, datenIn)
%
% 
% 
...
%
% The function plugin_discr_em is part of the MATLAB toolbox Gait-CAD. 
...
bezeichner = strcat('TERMQ', num2str([1:anz_em]'));
info = struct('beschreibung', 'TS->DISCR SF MEAN', 'bezeichner', bezeichner, ...
   'anz_zr', 0, 'anz_em', anz_em, 'laenge_zr', 0, 'typ', 'SF');
info.einzug_OK = 1; 
info.richtung_entfernen = 0;
info.anz_benoetigt_zr = 1;

info.explanation       = 'computes the frequencies for  (crisp) discretizations in time series.';
%info.explanation_long  = strcat(,' Further options ')
...
mode_fuzzy_plugin_em_aggr='MEAN DISCR';
