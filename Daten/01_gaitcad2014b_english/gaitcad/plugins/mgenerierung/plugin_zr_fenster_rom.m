  function [datenOut, ret, info] = plugin_zr_fenster_rom(paras, datenIn)
% function [datenOut, ret, info] = plugin_zr_fenster_rom(paras, datenIn)
%
%   Berechne das Maximum einer Zeitreihe.
%   Plugin-Fkt für Gait-CAD.
%
% The function plugin_zr_fenster_rom is part of the MATLAB toolbox Gait-CAD. 
...
info = struct('beschreibung', 'ROM of a window', 'bezeichner', 'FE-ROM', 'anz_zr', 1, 'anz_em', 0, 'laenge_zr', paras.par.laenge_zeitreihe, 'typ', 'TS');
info.einzug_OK = 0;
info.richtung_entfernen = 0;
info.anz_benoetigt_zr = 1;

info.explanation = strcat('sets all values of the time series in the window to the range of the window.',' Here, a causal filter is used.');
...
for i = 1:size(datenIn.dat,2)
   ...
   min_ = min(datenIn.dat(:, start:ende), [], 2);
   max_ = max(datenIn.dat(:, start:ende), [], 2);
   datenOut.dat_zr(:, i) = max_ - min_;
end; % for(i = 1:size(datenIn.dat,2))

ret.bezeichner = sprintf('FE-ROM %d', paras.parameter_commandline{1});
