  function [datenOut, ret, info] = plugin_normzahl_mittelwert(paras, datenIn)
% function [datenOut, ret, info] = plugin_normzahl_mittelwert(paras, datenIn)
%
% Plugin-Fkt für Gait-CAD.
%
% The function plugin_normzahl_mittelwert is part of the MATLAB toolbox Gait-CAD. 
...
info = struct('beschreibung', 'below/above norm mean value', 'bezeichner', 'ND Sign', 'anz_zr',0, 'anz_em', 1, 'laenge_zr', paras.par.laenge_zeitreihe, 'typ', 'SF');
info.einzug_OK = 1; 
info.richtung_entfernen = 0;
info.anz_benoetigt_zr = 1;

info.explanation      = 'computes a single feature indicating values above or below the norm time series';
...
if (~isfield(datenIn, 'ref') || isempty(datenIn.ref))
   mywarning('No normative data found. Remove single feature!');
