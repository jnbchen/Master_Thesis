  function [datenOut, ret, info] = plugin_geschwindigkeit_kausal(paras, datenIn)
% function [datenOut, ret, info] = plugin_geschwindigkeit_kausal(paras, datenIn)
%
%  Berechne die Beschleunigungszeitreihen
%
% The function plugin_geschwindigkeit_kausal is part of the MATLAB toolbox Gait-CAD. 
...
anz_zr = 1;
info = struct('beschreibung', 'Velocity (causal)', 'bezeichner', 'V_kausal', 'anz_zr', anz_zr, 'anz_em', 0, 'laenge_zr', paras.par.laenge_zeitreihe, 'typ', 'TS');
info.einzug_OK = 0; info.richtung_entfernen = 0;
info.anz_benoetigt_zr = 1;

info.explanation      = strcat('computes the first time derivation of a time series.',' The result is a single time series.',' Here, a causal filter is used.');
...
% Berechne die in Geschwindigkeit durch einen kausalen Ansatz delta y[k+1] = y[k+1] - y[k]. Der letzte Wert wird dupliziert
geschw=[zeros(size(datenIn.dat,1),1) datenIn.dat(:,2:size(datenIn.dat,2)) - datenIn.dat(:,1:size(datenIn.dat,2)-1) ];
