  function [datenOut, ret, info] = plugin_zr_fenster_median(paras, datenIn)
% function [datenOut, ret, info] = plugin_zr_fenster_median(paras, datenIn)
%
% 
% 
% 
%
% The function plugin_zr_fenster_median is part of the MATLAB toolbox Gait-CAD. 
...
info = struct('beschreibung', 'Median of a window', 'bezeichner', 'FE-MED', 'anz_zr', 1, 'anz_em', 0, 'laenge_zr', paras.par.laenge_zeitreihe, 'typ', 'TS');
info.einzug_OK = 0; 
info.richtung_entfernen = 0;
info.anz_benoetigt_zr = 1;

info.explanation = strcat('sets all values of the time series in the window to the median value of the window.',' Here, a causal filter is used.');
...
ret.bezeichner = sprintf('FE-MED %d', paras.parameter_commandline{1});
